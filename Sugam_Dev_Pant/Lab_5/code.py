...
def apply_scaling_around_point(x_coords, y_coords, scale_x, scale_y, fx, fy):
    """Scales the points around a fixed point (fx, fy)."""
    ...
    # Create the inverse translation matrix to move the points back
    ...
    # Composite transformation matrix: first move to origin, then scale, and finally move back
    composite_matrix = inverse_translation @ scaling_matrix @ translation_to_origin

    # Apply the scaling transformation to the original points
    return apply_2d_transformation(x_coords, y_coords, composite_matrix)
...
def plot_scaled_line():
    ...
    x_scaled, y_scaled = apply_scaling_around_point(x_orig, y_orig, scale_x, scale_y, x0, y0)

    # Plot original and scaled lines
    plt.figure(figsize=(8, 6))
    plt.plot(x_orig, y_orig, marker='o', color='blue', linestyle='-', label='Original Line')
    plt.plot(x_scaled, y_scaled, color='red', linestyle='--', label='Scaled Line')
    plt.title("Bresenham Line with Scaling Around Starting Point")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    plt.grid(True)
